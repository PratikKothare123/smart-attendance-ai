...
router.get('/', protect, async (req,res) => {
  try {
    const q={};
    ['year','semester','section','department'].forEach(k=>{ if(req.query[k]) q[k]=req.query[k]; });
...
router.get('/my-profile', protect, async (req,res) => {
  try {
    const s = await Student.findOne({ usn:req.user.usn }).select('-faceImages -faceEncoding');
    res.json(s);
  } catch(e){ res.status(500).json({ message:e.message }); }
});
...
router.post('/face/encode', protect, async (req,res) => {
  try {
    ...
    if(!usn||!images?.length) return res.status(400).json({ message:'USN and images required' });
    ...
    if(e.code==='ECONNREFUSED') return res.status(503).json({ message:'AI service offline. Start python main.py first.' });
